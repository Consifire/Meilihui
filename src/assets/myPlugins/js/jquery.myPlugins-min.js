;(function($) {$.fn.extend({myCarouselImg: function(options) {var defaults = {_width: 810,_height: 320,isShowPage: true,autoPlay: true,playType: 'left',
...
$page.on('mouseenter', 'span', function() {_index = $(this).index();startSwitch()});}}if(opt.isShowBtn) {$('<div/>').addClass('next').appendTo($self);
$('<div/>').addClass('prev').appendTo($self);$self.on('click', '.next', function() {_index++;startSwitch();}).on('click', '.prev', function() {
